...
static void respondRequest(struct UnixSocket *Socket)
{
    ...
    printf("Payload: ");
    for(int i = 0; i<sap->received.index; i++)
        printf("%c", sap->received.data->payload[i]);
    printf("\n");

    printf("respondRequest DataTransmitter \n");
...
static int dataTransmitterHandle(struct ChainBase *Base, uint8_t *Data, uint8_t Length)
{
